...
import static ro.bluebit.Diverse.Siruri.round;
...
public class ConvertNumar {
    public static String convert(Double nNumar ) {
        ...
        if (sInt.length() <= 12 ) {
            ...
            sCuvint = round(nNumar, 2) + "(Numar prea mare)" ;
        }
        return sCuvint ;
    }
    ...
    private static String zeci(String sSir ) {
        ...
        if (sSir.substring(0, 1).equals(" ")) zeci=cifre(sSir.substring(1,1+ 1), 1);
        else if (sSir.substring(0, 1).equals("0")) cifre(sSir.substring(1,1+ 1), 1);
        else if (sSir.substring(0, 1).equals("1")) {
            if (sSir.substring(1,1+1).equals("0"))
                zeci="zece";
            else
                zeci=cifre(sSir.substring(1,1+ 1), 2) + "sprezece" ;
        }
        else if (sSir.substring(0, 1).equals("2")) {
            if (sSir.substring(1,1+ 1).equals("0"))
                zeci = "douazeci";
            else
                zeci = "douazecisi" + cifre(sSir.substring(1, 1+1), 1);
        }
        else
            zeci = cifre(sSir.substring(0, 1), 2) + "zecisi" + cifre(sSir.substring(1,1+ 1), 1);
        return zeci ;
    }
